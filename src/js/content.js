...
function showGrayLayout() {
  var grayLayout = document.getElementById('__okta_grayLayout__');
  if (!grayLayout) {
    grayLayout = document.createElement('div');
    grayLayout.id = '__okta_grayLayout__';
    ...
    captureBox.id = '__okta_captureBox__';
...
function grayLayoutDown(event) {
  ...
  var captureBox = document.getElementById('__okta_captureBox__');
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  console.log('received message "%s" from %O', JSON.stringify(message), sender);
